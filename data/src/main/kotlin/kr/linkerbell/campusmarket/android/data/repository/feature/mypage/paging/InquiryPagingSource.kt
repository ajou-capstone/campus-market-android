...
class InquiryPagingSource(
...
) : PagingSource<Int, SummarizedUserReport>() {

    override fun getRefreshKey(state: PagingState<Int, SummarizedUserReport>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, SummarizedUserReport> {
        val pageNum = params.key ?: DEFAULT_PAGE_START
        val pageSize = params.loadSize

        return myPageApi.getReportList(
            page = pageNum,
            size = pageSize,
        ).map { data ->
